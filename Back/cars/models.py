...
class Cars(models.Model):
    ...
    garageName = models.CharField(max_length=20, null=True,blank=True)
    garagePhone = models.CharField(max_length=12, null=True,blank=True)
...
class Shifts(models.Model):
    ...
    # רק במקרה של מוסף נרדש שתי אנשים
    ...
    maintenanceType = models.ForeignKey(
        MaintenanceTypes, on_delete=models.CASCADE, null=True)
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=['shiftDate', 'car', 'maintenanceType'], name='my_shift_pk'),
...
def get_maintenance_upload_path(instance, filename):
    # Assuming you want to use the field 'field_in_foreign_table' as the upload path
    upload_path = instance.car_fileFolderName
    return f'{upload_path}/{filename}'
    # return f'maintenance/{upload_path}/{filename}'
...
class CarMaintenance(models.Model):
    ...
    fileMaintenance = models.FileField(upload_to = get_maintenance_upload_path, max_length=200, blank=True)
    # fileMaintenance = models.FileField(upload_to='maintenance/', max_length=200, blank=True)
    fileType = models.ForeignKey(
        FileTypes, on_delete=models.CASCADE, null=True)
